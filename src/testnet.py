...
# endrange = 131886954
endrange = 171886954
train_df = pd.read_csv(path+"train.csv", dtype=dtypes, skiprows=range(startrange, endrange), usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
...
model.fit(train_df, y_train, batch_size=batch_size, epochs=2, class_weight=class_weight, shuffle=True, verbose=1)
...
del test_df; gc.collect()
print("writing....")
sub.to_csv('testnet-submission.csv',index=False)
