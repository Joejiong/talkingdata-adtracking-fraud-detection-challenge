...
class DenseModelThree:
    ...
    def fit(self, X_train, Y_train, X_test, modeldir=".", logdir=".", epochs=2, batch_size=20000):
        ...
        # Compile the model
        lr = DenseModelThree.OPT_LEARNING_RATE
        ...
        CHK = ModelCheckpoint(
            ...
            mode="auto", period=1)
        ES = EarlyStopping(
            monitor="val_loss", min_delta=DenseModelThree.ES_MIN_DELTA,
            patience=DenseModelThree.ES_PATIENCE, verbose=2, mode="auto")
        TB = TensorBoard(
            log_dir=logdir, histogram_freq=0,
            write_graph=True, write_images=False)
        RLRP = ReduceLROnPlateau(
            monitor="val_loss", factor=DenseModelThree.RLRP_FACTOR,
            patience=DenseModelThree.RLRP_PATIENCE, min_lr=DenseModelThree.RLRP_MIN_LR,
            verbose=1)
        BL = BaseLogger()
