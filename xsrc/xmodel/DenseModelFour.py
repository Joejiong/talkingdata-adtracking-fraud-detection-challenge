...
from keras.layers import BatchNormalization, SpatialDropout1D, GaussianDropout, MaxPooling1D
...
class DenseModelFour:
    ...
    def fit(self, X_train, Y_train, X_test, modeldir=".", logdir=".", epochs=2, batch_size=20000):
        ...
        CHK = ModelCheckpoint(
            ...
            mode="auto", period=1)
        ES = EarlyStopping(
            monitor="val_loss", min_delta=DenseModelFour.ES_MIN_DELTA,
            patience=DenseModelFour.ES_PATIENCE, verbose=2, mode="auto")
        TB = TensorBoard(
            log_dir=logdir, histogram_freq=0,
            write_graph=True, write_images=False)
        RLRP = ReduceLROnPlateau(
            monitor="val_loss", factor=DenseModelFour.RLRP_FACTOR,
            patience=DenseModelFour.RLRP_PATIENCE, min_lr=DenseModelFour.RLRP_MIN_LR,
            verbose=1)
        BL = BaseLogger()
